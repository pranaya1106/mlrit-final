...
export default function Stats() {
  return (
    ...
    >
      ...
      <div aria-hidden className="ghost-numeral top-16 right-8 md:right-16">
        01
      </div>

      <div className="relative mx-auto max-w-[1440px] px-6 md:px-10 lg:px-12 pt-0 pb-6 md:pb-10">

        {/* The 4 ledger numbers */}
        <Stagger className="grid grid-cols-2 md:grid-cols-4 gap-x-6 md:gap-x-10 gap-y-12" delay={0.08}>
          {STATS.map((s, i) => (
            <StaggerItem key={i}>
              <StatItem index={i} {...s} />
            </StaggerItem>
...
function StatItem({ target, suffix, label, caption, footnote, index }: Stat & { index: number }) {
  ...
  return (
    <div ref={ref} className="flex flex-col items-start border-t-2 border-border/70 pt-5 group relative">
      ...
      <div className="flex items-baseline gap-1 text-foreground">
        <span className="font-sans font-black leading-none tracking-tighter-3 text-[clamp(3.4rem,5.6vw,4.8rem)]">
          {count}
        </span>
        <span className="editorial-italic text-primary leading-none text-[clamp(2rem,3vw,2.6rem)]">
          {suffix}
        </span>
      </div>
      <div className="mt-4 font-sans font-semibold text-[1.02rem] text-foreground/85 leading-snug">
        {label}
      </div>
      <div className="mt-1.5 text-muted text-[0.82rem] leading-snug">
        <span className="text-primary font-mono font-bold mr-1">†</span>
        {footnote}
      </div>
    </div>
  );
