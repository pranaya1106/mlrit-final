...
// No event photography exists for this club yet in lib/clubs.ts — same as
// the site's generic club template, each event is its own gradient tile
// rather than a cropped/guessed placeholder photo.
const EVENTS: EventItem[] = [
  { slug: 'code-hack-night', name: 'Hack Night', tag: 'Flagship', caption: 'An overnight build event where teams ship a working project from scratch by sunrise.', gradient: 'linear-gradient(155deg, #023d10 0%, #01741f 55%, #0a3d1f 100%)' },
  { slug: 'code-debug-clinic', name: 'Debug Clinic', tag: 'Recurring', caption: 'A weekly drop-in clinic where stuck code finally gets unstuck, with a senior pairing alongside.', gradient: 'linear-gradient(155deg, #0b1f3d 0%, #1e3a5f 55%, #14294a 100%)' },
  { slug: 'code-placement-drive', name: 'Placement Prep Drive', tag: 'Prep series', caption: 'A multi-week series covering the DSA and system design rounds that show up in real placement drives.', gradient: 'linear-gradient(155deg, #3a1503 0%, #b45309 55%, #7a3706 100%)' },
  { slug: 'code-first-commit', name: 'First Commit', tag: 'First-years', caption: 'A beginner-friendly session walking first-years through Git, GitHub, and their very first pull request.', gradient: 'linear-gradient(155deg, #1a0b3d 0%, #6b3fa0 55%, #3a1f5f 100%)' },
];
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: 'linear-gradient(to right, rgba(6,10,6,0.88) 0%, rgba(6,10,6,0.55) 45%, rgba(6,10,6,0.25) 100%)' }}
      />
    </motion.div>
  );
}
...
function Thumb({ item, active, onClick }: { item: EventItem; active: boolean; onClick: () => void }) {
  return (
    ...
    >
      ...
      {active && (
        <motion.div
          layoutId="code-thumb-ring"
          className="absolute inset-0 rounded-lg"
          style={{ border: `2px solid ${CODE_GREEN}`, willChange: 'transform' }}
...
// ─── Main ─────────────────────────────────────────────────────────────────────
export default function CodeEventsGallery() {
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div
                ...
                style={{ background: `${CODE_GREEN}22`, border: `1px solid ${CODE_GREEN}55`, color: CODE_GREEN }}
              >
                {active.tag}
              </div>
              <h2
            ...
            {EVENTS.map((_, i) => (
              <motion.div
                ...
                animate={{
                  ...
                  backgroundColor: i === activeIdx ? CODE_GREEN : '#fff',
                }}
