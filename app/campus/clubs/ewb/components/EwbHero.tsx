...
export default function EwbHero() {
  ...
  return (
    <section
      ...
      aria-label="EWB-IUCEE-IEEE Club Hero"
    >
      {/* Video background — the chapter's own motion poster.
          Source is shot on a light paper backdrop (unlike the other club heroes,
          which are dark to begin with) — a uniform dark wash keeps it consistent
          with the rest of this dark-themed page and keeps the white nav legible. */}
      <motion.div
        style={{ opacity: heroOpacity }}
        className="absolute inset-0 z-[1]"
      >
        <video
          src="/videos/ewb-hero.mp4"
          autoPlay
          muted
          loop
          playsInline
          ...
          aria-hidden="true"
        />
        {/* Uniform dark wash — the source video is on light paper, not black */}
        <div aria-hidden className="absolute inset-0" style={{ background: 'rgba(6,10,7,0.5)' }} />
        {/* Vignette */}
        <div
          aria-hidden
          className="absolute inset-0"
          style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0.5) 0%, transparent 30%, transparent 60%, rgba(0,0,0,0.75) 100%)' }}
        />
      </motion.div>
      ...
      <div className="absolute top-0 left-0 right-0 z-20 max-w-[1440px] mx-auto px-6 md:px-10 lg:px-16 pt-6 md:pt-8 flex items-center justify-between">
        ...
        >
          MLRIT · EWB-IUCEE-IEEE · Student Chapter
        </motion.span>
      </div>
